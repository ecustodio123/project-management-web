...
export function RegisterForm() {
  ...
  const isConfirming = Boolean(pendingEmail);
    ...
    >
      <Stack spacing={1}>
        <Typography
          ...
          variant="body2"
        >
          {isConfirming ? "Revisa tu correo" : "Empieza a organizar tu trabajo"}
        </Typography>
        <Typography variant="h4" sx={{ fontWeight: 800, lineHeight: 1.15 }}>
          {isConfirming ? "Confirma tu cuenta" : "Crea tu cuenta"}
        </Typography>
        <Typography color="text.secondary">
          {isConfirming
            ? `Enviamos un código de verificación a ${pendingEmail}.`
            : "Configura tu acceso para gestionar proyectos, tareas y clientes."}
        </Typography>
      </Stack>

      {signUpMutation.isError || confirmMutation.isError ? (
        <Alert severity="error" sx={{ borderRadius: 2 }}>
          {getErrorMessage(signUpMutation.error || confirmMutation.error)}
        </Alert>
      ) : null}
      ...
      {isConfirming ? (
        <Stack spacing={0.75}>
          ...
          >
            ...
            <Typography sx={{ fontWeight: 800 }}>Código enviado</Typography>
        ...
        {isConfirming ? (
          <>
            <Button
              ...
              variant="contained"
              size="large"
              disabled={!confirmationCode.trim() || confirmMutation.isPending}
